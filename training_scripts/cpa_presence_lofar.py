"""
Script for analyzing the impact of Closest Point of Approach (CPA) on classification using SVM LOFAR.
"""
import typing
import itertools
import argparse
...
def main(override: bool,
    ...
    output_base_dir = f"{DEFAULT_DIRECTORIES.training_dir}/{exp_str}/lofar_exp_{cpa_test}"

    classifiers = [iara_default.Classifier.SVM]

    manager_dict_0 = iara_default.default_lofar_managers(
                config_name = f'{str(collections[0])}',
                output_base_dir = output_base_dir,
                classifiers = classifiers,
                collection = iara_default.default_collection(only_sample=only_sample,
                                                             collection=collections[0]),
                data_processor = iara_default.default_iara_lofar_audio_processor(),
                training_strategy = training_strategy)

    manager_dict_1 = iara_default.default_lofar_managers(
    ...
    print('########## final grid ##########')
    print(merged_df)
    merged_df.to_latex(os.path.join(output_base_dir, 'lofar_cpa_proximity.tex' if cpa_test == 1 else 'lofar_cpa_presence.tex'), index=False)


if __name__ == "__main__":
    strategy_choises = [str(i) for i in iara_trn.ModelTrainingStrategy]

    parser = argparse.ArgumentParser(description='RUN SVM LOFAR CPA proximity search')
    parser.add_argument('--override', action='store_true', default=False,
                        help='Ignore old runs')
    parser.add_argument('--only_sample', action='store_true', default=False,
                        help='Execute only in sample_dataset.')
    ...
    parser.add_argument('-F', '--fold', type=str, default=None,
                        help='Specify folds to be executed.')
    test_choices=[1]
    parser.add_argument('--cpa_test', type=int, choices=test_choices, default=1,
