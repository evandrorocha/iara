"""
Training and Evaluation of a Hybrid Binary GRU Specialist (SMALL vs. LARGE)
using temporal sequences of concatenated MEL (256) + LOFAR-PCA (8) = 264 features.
...
def apply_spec_augment(X: torch.Tensor, time_mask_max: int = 2, freq_mask_max: int = 16, noise_std: float = 0.0) -> torch.Tensor:
    """
    Apply SpecAugment (Time & Frequency Masking) and Gaussian Noise Jitter to training batch.
...
def train_epoch(model, dataloader, optimizer, criterion, device, spec_augment: bool = False, noise_jitter: float = 0.0, time_mask_max: int = 2, freq_mask_max: int = 16):
    model.train()
    total_loss = 0.0
    total_samples = 0
...
class HybridSequenceAudioDataset(Dataset):
    """
    Dataset that constructs sequences from pre-extracted transformed features per file.
    """
...
def build_transformed_features(loader_mel, loader_lofar, file_ids_train, file_ids_all, n_pca=8, lofar_only=False):
    """
    Transform files into feature map {fid: (n_windows, input_dim)}.
    If n_pca > 0: concatenates MEL (256) + LOFAR-PCA (n_pca).
    If n_pca == 0 and not lofar_only: concatenates MEL (256) + full LOFAR (1024) = 1280 features.
    If lofar_only: uses full LOFAR (1024) features alone.
    """
    if not lofar_only:
        loader_mel.pre_load(file_ids_all)
    loader_lofar.pre_load(file_ids_all)

    if n_pca > 0 and not lofar_only:
        lofar_train_list = []
    ...
    for fid in file_ids_train:
        ...
        if lof_t is None or len(lof_t) == 0:
            continue
        lof = lof_t.numpy()
        if lofar_only:
            feat = lof
            ...
            if mel_t is None or len(mel_t) == 0:
                continue
            mel = mel_t.numpy()
...
def main():
    ...
    parser.add_argument("--n_pca", type=int, default=0, help="Number of LOFAR PCA components (0 = full LOFAR 1024)")
    parser.add_argument("--lofar_only", action="store_true", help="Use full LOFAR 1024 only (no MEL)")
    ...
    parser.add_argument("--spec_augment", action="store_true", default=False, help="Enable SpecAugment (Time & Frequency Masking)")
    parser.add_argument("--noise_jitter", type=float, default=0.0, help="Gaussian noise standard deviation (0.0 = disabled)")
    parser.add_argument("--time_mask_max", type=int, default=2, help="Max time steps to mask")
    parser.add_argument("--freq_mask_max", type=int, default=16, help="Max frequency bins to mask")
    ...
    exp_name = args.save_dir if args.save_dir else f"gru_sl_{mode_str}_s{args.stride}_h{args.hidden_size}_l{args.num_layers}_lr{args.lr}"
    ...
    split_list = exp_config_lofar.split_datasets() if args.lofar_only else exp_config_mel.split_datasets()
    ...
    print(f"\n{'='*75}")
    print(f" Treinamento Especialista GRU ({mode_str} = {input_dim} dimensões)")
    ...
    for fold_idx in fold_list:
        print(f"\n>>> [Fold {fold_idx}/10] Preparando features híbridas (PCA{args.n_pca} + Normalização)...")
        ...
        train_ds = HybridSequenceAudioDataset(
            feat_map, train_fids, trn_df['Target'].tolist(),
            seq_len=args.seq_len, stride=args.stride
        )
        val_ds = HybridSequenceAudioDataset(
            feat_map, val_fids, val_df['Target'].tolist(),
            seq_len=args.seq_len, stride=args.stride
        )
        test_ds = HybridSequenceAudioDataset(
