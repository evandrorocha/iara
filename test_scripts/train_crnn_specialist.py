"""
Training and Evaluation of a Binary CRNN Specialist (SMALL vs. LARGE)
Architecture: Conv1D Spectral Feature Extractor -> Bi-GRU -> Temporal Attention -> Classification Head.
...
import iara.ml.models.crnn as iara_crnn
...
def main():
    parser = argparse.ArgumentParser(description="Train CRNN SL Specialist")
    ...
    parser.add_argument("--conv_channels", type=str, default="32,64", help="Conv1D channels, comma-separated")
    parser.add_argument("--conv_kernel", type=int, default=5, help="Conv1D kernel size")
    ...
    exp_name = args.save_dir if args.save_dir else f"crnn_sl_{mode_str}_s{args.stride}_conv{args.conv_channels}_h{args.gru_hidden}_lr{args.lr}"
    ...
    print(f"\n{'='*75}")
    print(f" Treinamento Especialista CRNN (Conv1D + Bi-GRU + Attention) — {mode_str} ({input_dim} dims)")
    ...
    for fold_idx in fold_list:
        print(f"\n>>> [Fold {fold_idx}/10] Preparando features ({mode_str})...")
        ...
        torch.save(best_model_state, os.path.join(fold_model_dir, "crnn_model.pt"))
