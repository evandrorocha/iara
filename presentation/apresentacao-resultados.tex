...
\begin{frame}{Protocolo Experimental $5\times2\text{cv}$: Treino, Valida\c{c}\~{a}o e Teste}
    ...
    \begin{columns}[t]
        ...
        \begin{block}{\small Protocolo $5\times2\text{cv}$ (10 Folds de Teste)}
            \scriptsize
            \begin{itemize}
                \item \textbf{Exclusive Ships on Test}: Divis\~{a}o estratificada por navio f\'{i}sico (ID de arquivo). Nenhum navio de teste est\'{a} no treino (\textit{zero data leakage}).
                \item \textbf{Volume de Teste}: $639\text{ \'{a}udios} \times 10\text{ folds} = \mathbf{6.390\text{ avalia\c{c}\~{o}es}}$.
                \item Matrizes de confus\~{a}o e \'{I}ndice SP consolidados sobre essas 6.390 predi\c{c}\~{o}es de teste.
            \end{itemize}
        \end{block}
    \end{columns}
\end{frame}
...
\begin{frame}{Distribui\c{c}\~{a}o das Classes na Parti\c{c}\~{a}o de Teste (639 \'{A}udios)}
    ...
    \midrule
    \textbf{Total}      & ---                    & $\mathbf{639}$ & $\mathbf{100,0\%}$ & $\mathbf{6.390}$ & $>64$ horas de grava\c{c}\~{o}es de teste \\
    \bottomrule
    \end{tabular}%
    }
    \end{table}
...
\begin{frame}{An\'{a}lise de Incerteza e Op\c{c}\~{a}o de Rejei\c{c}\~{a}o (Tabela 9 do Artigo)}
    ...
    \begin{columns}
        ...
        \begin{itemize}
            ...
            \item \textbf{LOFAR:} Alta Precis\~{a}o  sob alto consenso ($>90\%$).
        \end{itemize}
...
\begin{frame}{Evolu\c{c}\~{a}o dos Classificadores Especialistas SL (SMALL vs. LARGE)}
    ...
    \midrule
    SVM H\'{i}brido (PCA-4) & MEL-256 + LOFAR-PCA4 (260-D) & 6000 & $78,5 \pm 2,1$ & Poucas componentes para capturar o diesel \\
    SVM H\'{i}brido (PCA-16) & MEL-256 + LOFAR-PCA16 (272-D) & 6000 & $80,4 \pm 1,9$ & Leve excesso de vari\^{a}ncia espectral \\
    \midrule
    \textbf{Melhor Especialista SL} & \textbf{MEL-256 + LOFAR-PCA8 (264-D)} & \textbf{6000} & $\mathbf{83,8 \pm 1,4}$ & \textbf{Equil\'{i}brio \'{O}timo: MEL + LOFAR Compacto} \\
    \bottomrule
    \end{tabular}%
    }
    \end{table}
...
\begin{frame}{Arquitetura do Especialista SL: Fus\~{a}o H\'{i}brida (MEL-256 + LOFAR-PCA8)}
    \footnotesize
    O especialista bin\'{a}rio combina a pot\^{e}ncia do envelope cont\'{i}nuo com as raias discretas de cavita\c{c}\~{a}o:
    
    \vspace{0.08cm}
    \centering
    \includegraphics[width=0.78\textwidth,height=0.52\textheight,keepaspectratio]{imgs/fusao_hibrida_mel_lofar_pt.jpg}
    
    \vspace{0.05cm}
    \begin{columns}[t]
        \column{0.49\textwidth}
        \begin{block}{\scriptsize 1. Sinergia Espectral (Vetor de 264-D)}
            \tiny
            \textbf{MEL (256-D):} Fornece a energia hidrodin\^{a}mica e o envelope global.\\
            \textbf{LOFAR-PCA (8-D):} Adiciona as 8 raias harm\^{o}nicas dominantes do motor diesel.
        \end{block}
        
        \column{0.49\textwidth}
        \begin{block}{\scriptsize 2. Filtragem}
            \tiny
            O PCA reduz os 1.024 bins esparsos para 8 atributos cr\'{i}ticos, descartando ru\'{i}do e elevando a precis\~{a}o em LARGE para $\mathbf{83,8\%}$ no SVM Nystr\"{o}m ($m=6000$).
        \end{block}
    \end{columns}
\end{frame}
