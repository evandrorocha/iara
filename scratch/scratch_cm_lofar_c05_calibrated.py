...
for fold in range(10):
    ...
    # Read the calibrated file
    csv_path = os.path.join(fold_dir, f"{exp_name}_multiclass_test_b0.32_0.27_0_0.csv")
...
# Print raw matrix
print(f"=== MATRIZ DE CONFUSÃO CALIBRADA ({folds_found} folds) ===")
...
print("\n=== MATRIZ DE CONFUSÃO NORMALIZADA CALIBRADA (RECALL %) ===")
print(f"{header_title:<12} | " + " | ".join(f"{name:<10}" for name in class_names))
print("-" * 60)
for i, name in enumerate(class_names):
    row_sum = sum(cm[i])
